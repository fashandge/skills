...
For quizzes, each answer should be the same number of words (and characters, if possible). Don't give the user any clues about the answer through formatting. **Crucially, the VISIBLE TEXT of every option MUST be different** — the same-length constraint applies to the answer strings, not to making them identical. An option is a genuinely different possible answer rendered in roughly the same character budget. Pre-flight checklist before writing a quiz block: (1) are all four visible options meaningfully different texts? (2) is each a plausible but incorrect answer for the wrong option? (3) are they all the same ~word count so the correct answer doesn't stand out by length?
