...
description: >
  Explain a concept by motivating it with vivid minimal examples in the Kaplan style: start with a concrete scenario, surface the intuitive verdict, name the distinction it forces, then introduce formalism as bookkeeping. Use when the user asks for intuition or examples, e.g. "explain X with examples", "give me the intuition for X", "motivate X", "make X click", "ELI5 but rigorous", or "/motivating-examples X". Prefer this over socratic-tutor when the user wants to be shown rather than questioned, and over deep-mastery when they want the idea to click rather than be drilled.
---
